...
class PostHandler(QtCore.QThread):
    def __init__(self, postQueue, sysQueue, instr, quantity=1, parent=None):
    ...
    def get_answr(self, pr, req, cur):
        #get object wich done reqwest -> if there is no object "return 0"
        try:
            obj = req["obj"]
        except:
            return None
            #system logging -> incorrect request: missing obligatory element ::"obj"
        ...
        try:
            ...
            return None
            #system logging -> incorrect request: missing obligatory element ::"req"
        ...
        if not exe == "":
            ...
            for key in exe:
                try:
                    ...
                    #system logging -> incorrect accessing to dataBase
                    return None
            return ans
        

    def req_hendler(self, pr, req, cur):
        ans = self.get_answr(pr, req, cur)
        f_ans = ""
        if not ans is None:
            for key in ans:
                f_ans += str(ans[key])
           ...
           # conn = psycopg2.connect(get_command("connection").format('switch_core', 'local', 'localhost', '123'))
            ...
            while not self._stopped:
                if not self.queue.empty():
                    ...
                    ans = self.req_hendler(pr, req, cur)
                    if not ans is None:
                        obj = req["obj"]
                        obj.post_recive(ans)
